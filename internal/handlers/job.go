...
func (h *JobHandler) CreateJob(c *gin.Context) {
	...
	if err == nil {
		...
		// Read uploaded file content, process it to strip whitespace, then save
		...
		// Process the content to strip whitespace from each line
		processedContent := processHashContent(string(content))

		// Write the processed content to the file
		if err := os.WriteFile(hashesFile, []byte(processedContent), 0644); err != nil {
	...
	if hashesText != "" {
		...
		if content, err := os.ReadFile(hashesFile); err == nil {
			totalHashes = len(splitLines(string(content)))
		}
	}
...
func (h *JobHandler) CancelJob(c *gin.Context) {
	...
	// Clean up wordlist chunks immediately when job is cancelled
	splitter := wordlistpkg.NewWordlistSplitter(h.uploadPath)
	go func() {
		if err := splitter.CleanupChunks(job.UID); err != nil {
			...
			log.Printf("Successfully cleaned up wordlist chunks for cancelled job %s", job.UID)
		}
	}()
