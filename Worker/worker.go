...
type JobAssignment struct {
	...
	UsernameMode   bool   `json:"username_mode"`
...
func (hm *HashcatManager) runHashcatShow(assignment JobAssignment, hashFile string) {
	...
	// Add username mode if requested
	if assignment.UsernameMode {
		showArgs = append(showArgs, "--username")
	}

	showArgs = append(showArgs, hashFile)
...
func (hm *HashcatManager) checkForJobCancellations() {
	...
	for jobKey, process := range hm.processes {
		...
		if resp.StatusCode == http.StatusOK {
			...
			// If not assigned anymore, check if it's cancelled or just completed
			if !stillAssigned {
				// Check if the process is already finished (completed/exhausted/failed)
				// Don't mark already-finished jobs as cancelled
				if process.Status == "completed" || process.Status == "exhausted" ||
				   process.Status == "failed" || process.Status == "aborted" {
					log.Printf("Job %s chunk %d is no longer assigned but already finished with status: %s",
						process.JobUID, process.ChunkIndex, process.Status)
					continue
				}
...
func (hm *HashcatManager) checkForNewJobs() {
	...
	for _, assignment := range response.Assignments {
		...
		// Skip if already in overloaded queue (waiting for retry)
		if _, exists := hm.overloadedJobs[jobKey]; exists {
			log.Printf("Skipping job %s - already in overloaded retry queue", jobKey)
			continue
		}

		// Start job directly - let hashcat determine if overloaded via exit code 252
...
func (hm *HashcatManager) startJobChunk(assignment JobAssignment) {
	...
	if assignment.HashesText != "" {
		...
		log.Printf("No hashes provided in assignment")
		hm.updateChunkStatus(assignment.JobUID, assignment.ChunkIndex, "failed")
		return
	}
			...
			if process.Cancelled {
				...
				if exitError, ok := err.(*exec.ExitError); ok {
					...
					case 252:
						...
						// Remove from processedJobs so it can be retried
						jobKey := fmt.Sprintf("%s_%d", assignment.JobUID, assignment.ChunkIndex)
						delete(hm.processedJobs, jobKey)

						hm.queueOverloadedJob(assignment, wordlistPath, currentProcs)
						// Report queued status to server so UI can show worker is waiting
						hm.reportProgress(&models.HashcatProgress{
							JobUID:        assignment.JobUID,
							Progress:      0,
							KeyspaceSize:  0,
...
func (hm *HashcatManager) checkOverloadedJobs() {
	...
	for jobKey, overloadedJob := range hm.overloadedJobs {
		...
		// Wait for process count to drop by at least 1 from when it was overloaded
		// This ensures hashcat has capacity to try again
		if currentProcs >= overloadedJob.OverloadThreshold {
			log.Printf("Job %s still waiting: current=%d, needs<%d (must drop by at least 1)", jobKey, currentProcs, overloadedJob.OverloadThreshold)
			continue
		}
