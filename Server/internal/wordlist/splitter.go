...
func (ws *WordlistSplitter) CountLines(wordlistPath string) (int64, error) {
	...
	// Increase buffer size to handle very long lines (10MB max per line)
	...
	// Check for scanner error, but skip "token too long" errors
	if err := scanner.Err(); err != nil && err.Error() != "bufio.Scanner: token too long" {
		return 0, err
	}
