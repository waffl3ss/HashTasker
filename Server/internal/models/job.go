...
type Job struct {
	...
	UsernameMode   bool           `gorm:"default:false" json:"username_mode"`  // Enable hashcat --username flag
	IsNTDS         bool           `gorm:"default:false" json:"is_ntds"`       // Flagged when hash_mode=1000 and >300 hashes
	WebhookURL     string         `gorm:"size:512" json:"webhook_url"`       // Optional webhook URL for job status notifications
...
type CrackedHash struct {
	ID           uint      `gorm:"primaryKey" json:"id"`
	JobID        uint      `gorm:"not null;uniqueIndex:idx_job_hash" json:"job_id"`
	Job          Job       `gorm:"foreignKey:JobID" json:"job"`
	Hash         string    `gorm:"not null;uniqueIndex:idx_job_hash" json:"hash"`
