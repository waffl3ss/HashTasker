...
func setupRouter(authService *auth.AuthService, serverConfig models.ServerConfig) *gin.Engine {
	...
	auth.Use(middleware.AuthRequired(authService))
	{
		...
		auth.GET("/admin/analytics", analyticsHandler.AnalyticsPage)
		...
		api.Use(middleware.JSONAuthRequired(authService))
		{
			...
			api.GET("/jobs/:uid/export/cracked", jobHandler.ExportCrackedHashesCSV)
			api.GET("/jobs/:uid/export/all", jobHandler.ExportAllHashesCSV)
			api.GET("/jobs/:uid/export/potfile", jobHandler.ExportPotfile)
			...
			// Admin analytics
			api.GET("/admin/analytics", analyticsHandler.GetAnalytics)
			api.DELETE("/admin/analytics", analyticsHandler.ClearAnalytics)
		}
	}
	...
	// Background task to auto-delete jobs older than 45 days
	// Run once on startup (catches up on old jobs immediately), then every 12 hours
	go func() {
		time.Sleep(10 * time.Second)
		jobHandler.AutoDeleteOldJobs()
	...
	// Background task to clean up orphaned files
	// Run after auto-delete on startup, then every hour
	go func() {
		time.Sleep(20 * time.Second) // Run after auto-delete has had a chance to finish
		workerHandler.CleanupOrphanedFiles()
